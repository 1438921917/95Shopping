...
-(void)jiekouPage:(NSString*)page{
    [Engine guanLiLieBiaoYeShu:page Stype:@"0" success:^(NSDictionary *dic) {
...
-(void)btnAddDianJi:(GuanLiCell*)cell{
    ...
    if (!cell.moreButtonClickedBlock) {
        ...
        [cell setMoreButtonClickedBlock:^(UIButton *btn,NSIndexPath *indexPath) {
            ...
            NSLog(@"输出%lu",btn.tag);
           //1.刷新 2.下架 3.置顶 4.成交
            [Engine gengXinChanPinStypeMessageID:md.messageID State:[NSString stringWithFormat:@"%lu",btn.tag+1] success:^(NSDictionary *dic) {
                NSString * item1 =[NSString stringWithFormat:@"%@",[dic objectForKey:@"Item1"]];
                if ([item1 isEqualToString:@"1"]) {
                    [LCProgressHUD showMessage:[dic objectForKey:@"Item2"]];
                }else{
